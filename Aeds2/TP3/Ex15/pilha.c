...
{
    printf("[%i] %s %s %02i/%02i/%04i %i %s %s %s %g [",
           j,
           movie->name,
           movie->original_title,
           movie->release_date.day, movie->release_date.month, movie->release_date.year,
...
void inserirPilha(PilhaFlexivel *pilha, Movie *elemento)
{
...
Movie removerPilha(PilhaFlexivel *pilha)
{
    ...
    Movie resp = pilha->topo->elemento;
...
void mostrarPilha(PilhaFlexivel *pilha)
{
    ...
    for (i = pilha->topo; i != NULL; i = i->prox)
    {
        printf("[%i] %s %s %02i/%02i/%04i %i %s %s %s %g [",
               contador,
               i->elemento.name,
               i->elemento.original_title,
               i->elemento.release_date.day, i->elemento.release_date.month, i->elemento.release_date.year,
               i->elemento.duration,
               i->elemento.genre,
               i->elemento.original_language,
               i->elemento.situation,
               i->elemento.budget);
        for (int j = 0; j < i->elemento.count_keywords; j++)
        {
            if (j == i->elemento.count_keywords - 1)
            {
                printf("%s]\n", i->elemento.keywords[j]);
            }
            else
            {
                printf("%s, ", i->elemento.keywords[j]);
            }
        }
        if (i->elemento.count_keywords == 0)
        {
            printf("]\n");
        }
        contador++;
...
void tratarComando(char *s, PilhaFlexivel *pilha)
{
    ...
    if (s[0] == 'I')
    {
        ...
        inserirPilha(pilha, &movies[count_movies++]);
    }
    else if (s[0] == 'R')
    {
...
int main()
{
    ...
    {
        movie_readHtml(line);
        inserirPilha(pilhaflexivel, &movies[count_movies++]);
        // printf("%s",pilhaflexivel->topo->elemento.name);
        readline(line + prefix_size, MAX_LINE_SIZE);
    }
    char comandos[200];
    scanf("%i", &numeroInsercoes);
    for (int j = 0; j < numeroInsercoes; j++)
    {
        scanf(" %[^\n]s", comandos);
        rtrim(comandos);
        tratarComando(comandos, pilhaflexivel);
        // mostrar();
    }
    printaRemovidos();
