...
void push(struct Node** head_ref, Movie new_data)
{
...
void insertAfter(struct Node* prev_node, Movie new_data)
{
...
void append(struct Node** head_ref, Movie new_data)
{
...
void mostrar(struct Node* node) {
    ...
    while (node != NULL) {
        ...
        contador,
        node->data.name,
        node->data.original_title,
        node->data.release_date.day,node->data.release_date.month,node->data.release_date.year,
        node->data.duration,
        node->data.genre,
        node->data.original_language,
        node->data.situation,
        node->data.budget);
        for (int j = 0; j < node->data.count_keywords; j++)
        {
            if (j == node->data.count_keywords -1)
            {
                printf("%s]\n",node->data.keywords[j]);
            }
            else
            {
                printf("%s, ",node->data.keywords[j]);
            } 
        }
        if (node->data.count_keywords == 0)
        {
            printf("]\n");
        }          
        last = node;
...
int main()
{
    ...
    {   
        movie_readHtml(line);
        append(&head,movies[count_movies++]);
        readline(line + prefix_size, MAX_LINE_SIZE);
    }
    mostrar(head);
