...
class lista {
    ...
    public void merge(Filme arr[], int l, int m, int r)
    {
        ...
        while (i < n1 && j < n2)
        {
            if (L[i].getOrcamento()<=R[j].getOrcamento())
            {
                arr[k] = L[i];
                i++;
            }
    ...
    // merge()
    public void sort(Filme arr[], int l, int r)
    {
        if (l < r)
        {
            int m = (l+r)/2;
    ...
    public static void swap(Filme[] vet, int i, int j) {
        Filme tmp;
        tmp = vet[i];
        vet[i] = vet[j];
        vet[j] = tmp;
