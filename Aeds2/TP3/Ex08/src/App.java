...
class lista 
{
    ...
    {   
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                if (lista[j].getDuracao() == lista[i].getDuracao()) 
                {
                    if (lista[j].getTituloOriginal().compareTo(lista[i].getTituloOriginal())<0) {
                        //a<b
                        //Lista[j] é vem primeiro que lista[i];
                        tmp = lista[j];
                        lista[j] = lista[i];
                        lista[i] = tmp;
                    }
                ...
                else if (lista[j].getDuracao()<lista[i].getDuracao()) 
                {
                    tmp = lista[i];
                    lista[i] = lista[j];
                    lista[j] = tmp;
...
public class App {
    ...
    {    
        ...
        Arq.openWrite("matrículabolha.txt");
